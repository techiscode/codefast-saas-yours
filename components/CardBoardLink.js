"use client";
//when you have a listener in a component, then you have to make it a client component.

import toast from "react-hot-toast";
//you use toast to displya a message
...
const CardBoardLink = ({ boardId }) => {
  const boardLink = `${
    process.env.NODE_ENV === "development"
      ? "http://localhost:3000"
      : "https://memberadvise.com"
  }/b/${boardId}`;

  const copyLink = () => {
    navigator.clipboard.writeText(boardLink);
    //navigator = the browser, we have access to clipboard and we can writeText
